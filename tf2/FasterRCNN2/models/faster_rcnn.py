...
def faster_rcnn_model(mode, num_classes):
  assert mode == "train" or mode == "infer"
  if mode == "train":
    return _training_model(num_classes)
  else:
...
def _training_model(num_classes):
  ...
  # Build model
  model = Model([ stage1_feature_extractor_model.input, image_shape_map, anchor_map, anchor_valid_map, gt_rpn_map, gt_box_class_idxs_map, gt_box_corners_map ], [ rpn_score_output, rpn_regression_output, proposals_output, rpn_class_loss, rpn_regression_loss ])
...
def _label_proposals(proposals, gt_box_class_idxs, gt_box_corners, num_classes, min_background_iou_threshold, min_object_iou_threshold):
    ...
    gt_classes = tf.one_hot(indices = gt_box_class_idxs, depth = num_proposals) # (N,num_classes)
...
def _sample_proposals(proposals, gt_classes, gt_regressions, max_proposals, positive_fraction):
  ...
  positive_indices = tf.squeeze(tf.where(class_indices > 0))  # (P,), tensor of P indices (the positive, non-background classes in class_indices)
  negative_indices = tf.squeeze(tf.where(class_indices <= 0)) # (N,), tensor of N indices (the negative, background classes in class_indices)
 ...
 #num_positive_samples <= 0 or num_negative_samples <= 0
